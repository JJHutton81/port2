export const tabData = [
  'All',
  'React',
  'Redux',
  // 'API', 
  // 'JavaScript'
];

export const cardData = [
  {
    headline: 'Join Our Big Day',
    tab: 'Redux',
    img: './assets/jobdthumb.png',
    plink: 'https://joinourbigday.netlify.com',
    rlink: 'https://github.com/Lambda-School-Labs/labspt2-wedding-website',
    text:
      'Join Our Big Day is a web app that helps users manage their wedding guest list. It allows them to create their invitations using a template, and send them out to their guests via email.',
    stack:
      'Built using: ReactJS, ReduxJS, Stripe, and Express, Styled-Components',
    bullet1: 'Five person build done in ten weeks',
    bullet2: 'I engeneered the UI/UX',
    bullet3: 'Hosted on Netlify for front-end and Heroku back-end.'
  },
  {
    headline: 'Note Keeping App',
    tab: 'React',
    img: './assets/note.png',
    plink: 'https://basicnoteapp.netlify.com',
    rlink: 'https://github.com/FreeKeyBoy/front-end-project-week',
    text:
      'A note-keeping app that allows you to add, edit or delete notes on a dynamic server.',
    stack: 'Built using: Reactjs, NodeJS, Express, Styled-Components',
    bullet1: 'Solo build',
    bullet2: 'Front-end hosted on Netlify.',
    bullet3: 'Back-end hosted on Heroku.'
  },
  {
    headline: 'Instagram Clone',
    tab: 'React',
    img: './assets/instaclone.jpg',
    plink: 'https://reactinsta.netlify.com',
    rlink: 'https://github.com/FreeKeyBoy/React-Insta-Clone',
    text:
      'This is an Instagram clone i made with ReactJS. There is no backend so the comments and like counter will reset when the page reloads.',
    stack: 'Built using: ReactJS, CSS',
    bullet1: 'Solo build',
    bullet2: 'Hosted on Netlify',
    bullet3: 'Uses JWS tokens'
  },
  {
    headline: 'React Sorting Project',
    tab: 'React',
    img: './assets/lambdatimes.jpg',
    plink: 'https://lambdatimesfilter.netlify.com',
    rlink: 'https://github.com/FreeKeyBoy/Sprint-Challenge-Lambda-Times-React',
    text:
      'This is a React app I made to learn how to sort components by category using key value pairs.',
    stack: 'Built using: ReactJS, CSS',
    bullet1: 'Solo build',
    bullet2: 'Hosted on Netlify',
    bullet3: 'Uses stateful components'
  },
  // {
  //   headline: 'JavaScript UI Project',
  //   tab: 'JavaScript',
  //   img: '/assets/newsfeed.jpg',
  //   plink: 'https://newsfeedcomp.netlify.com',
  //   rlink: 'https://github.com/FreeKeyBoy/Newsfeed-Components',
  //   text: 'This is a JavaScript project I made using Less, and HTML.',
  //   stack: 'Built using: ES6, HTML, CSS',
  //   bullet1: 'Solo build',
  //   bullet2: 'Hosted on Netlify'
  // },
  // {
  //   headline: 'Tabs Project JS',
  //   tab: 'JavaScript',
  //   img: '/assets/tabs.jpg',
  //   plink: 'https://tabsproject.netlify.com',
  //   rlink: 'https://github.com/FreeKeyBoy/Tabs-Components',
  //   text:
  //     'This is a JavaScript project I made using Less, and HTML. Enjoy my favorite Douglass Adams quotes.',
  //   stack: 'Built using: ES6, HTML, CSS',
  //   bullet1: 'Solo build',
  //   bullet2: 'Hosted on Netlify'
  // },
  // {
  //   headline: 'Swapi API Project',
  //   tab: 'API',
  //   img: '/assets/starwars.jpg',
  //   plink: 'https://reactwarsapi.netlify.com',
  //   rlink: 'https://github.com/FreeKeyBoy/Sprint-Challenge-React-Wars',
  //   text:
  //     'This is a ReactJS based app that was made using the swapi Star Wars API. I used pagination to sort out the results.',
  //   stack: 'Built using: ReactJS, CSS',
  //   bullet1: 'Solo build',
  //   bullet2: 'Hosted on Netlify'
  // }
];
